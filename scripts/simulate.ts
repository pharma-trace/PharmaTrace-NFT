...
async function main() {
  ...
  await expect(ptMarket.connect(admin).whitelistCurrency(ptToken.address, true)).to.emit(ptMarket, "CurrencyWhitelisted");
  await expect(ptMarket.connect(admin).whitelistCollection(ptCollection.address)).to.emit(ptMarket, "CollectionWhitelisted");
  ...
  await expect(
    ptMarket.connect(userB).withdrawOffer(ptCollection.address, voucher.tokenId)
  )
  ...
  await expect(ptMarket.connect(userB).createLazzOffer(ptCollection.address, voucher, OFFER_PRICE, {
    value: OFFER_PRICE,
  })).to.emit(ptMarket, "OfferCreated");
  ...
  await expect(
    ...
    .to.emit(ptMarket, "OfferCreated")
    .to.changeEtherBalances([ptMarket, userB, userC], [priceOverwrite.sub(OFFER_PRICE), OFFER_PRICE, priceOverwrite.mul(-1)]);
  ...
  await expect(ptMarket.connect(userA).acceptOffer(ptCollection.address, voucher.tokenId, true))
    ...
    .to.changeEtherBalances([ptMarket, admin, userA], [priceOverwrite.mul(-1), expectedFee, priceOverwrite.sub(expectedFee)]);

  console.log("Step 8. List Item");
  await expect(ptCollection.connect(userC).approve(ptMarket.address, voucher.tokenId)).to.emit(ptCollection, "Approval");
  await expect(
    ptMarket
      .connect(userC)
  ...
  await expect(
    ptMarket
      .connect(userC)
      .unlistItem(ptCollection.address, voucher.tokenId),
  ).to.emit(ptMarket, "ItemUnlisted");
  ...
  await expect(
    ptMarket.connect(userA).createOffer(ptCollection.address, voucher.tokenId, OFFER_PRICE, {
      value: OFFER_PRICE,
    }),
  )
    .to.emit(ptMarket, "OfferCreated")
  ...
  await expect(ptMarket.connect(admin).whitelistCurrency(ptToken.address, false)).to.emit(ptMarket, "CurrencyWhitelisted");
