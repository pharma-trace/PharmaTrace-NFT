...
contract PTMarket is IPTMarket, ReentrancyGuard {
    ...
    function createOfferFoRLazzNFT(
        ...
        IPTCollection(s_nftContractAddress).verifySignature(voucher); // this external call to function to verify the structure of vocher is valid
        ...
        checkERC20Requirment(_offerAmount, voucher.pid);
        checkRequirment(
            offer.status,
            offer.expiresAt,
            offer.offerAmount,
            voucher.minPrice,
            _offerAmount
        ); // verify values like is older offer is expired or not ...
        ...
        nonReentrant
    {
        ...
        address oldOfferBy = address(0);
        uint256 oldOfferValue = 0;
        address signer = IPTCollection(s_nftContractAddress).verifySignature(voucher); // get address of the owner from token signature
        ...
        checkRequirment(
            ...
            voucher.minPrice,
            _offerAmount
        ); // verify basic conditions are fullfil
        if (offer.expiresAt != 0 && offer.offerAmount != 0) {
        ...
        AllowedCrypto[voucher.pid].transferFrom(msg.sender, address(this), _offerAmount);
        IPTCollection(s_nftContractAddress).redeem(msg.sender, voucher); // external call to mint and transfer to new owner
        ...
        nonReentrant
    {
        ...
        address signer = IPTCollection(s_nftContractAddress).verifySignature(voucher);
        ...
        IPTCollection(s_nftContractAddress).redeem(offer.offerBy, voucher); // mine NFT and Transfer
        ...
        isListed(nftAddress, itemId)
    {
        ...
        // IPTCollection(s_nftContractAddress).revertApprovalForAll(address(0), item.tokenId);
        if (oldOfferValue != 0) {
            emit WithDrawRefundAmount(
                offer.tokenId,
                s_nftContractAddress,
    ...
    function createOffer(
        ...
        checkERC20Requirment(offerAmount, item.pid);
    ...
    function checkERC20Requirment(uint256 newOfferAmount, uint256 pid) internal view {
        ...
        if (AllowedCrypto[pid].allowance(address(this), msg.sender) == 0)
            revert PTNFTMarketPlace__PermissionRequired();
        if (AllowedCrypto[pid].allowance(address(this), msg.sender) < newOfferAmount)
